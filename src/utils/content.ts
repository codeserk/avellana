...
export type Collection = any // keyof ContentEntryMap
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export const COLLECTIONS: Collection[] = ['blog', 'page']
...
export async function getEntryLogs(collection: Collection, slug: string): Promise<Entry[]> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const entries = await getCollection(collection, (it: any) => it.slug.includes(`${entrySlug}/log/`))
